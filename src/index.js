...
async function run(queries, outputPath) {
    ...
    for (const query of queries) {
        ...
        if (imageContainer) {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                await downloadImage(src, query+i, outputPath);
                count++;
        ...
        console.log("Finished with ", count, " images iterated for: "+query)
        await page.close();
    }
...
const queries = ['cats', 'dogs', 'cars'];
const outputPath = "/Users/tudor/Documents/HVDD-RAW"

run(queries, outputPath);
